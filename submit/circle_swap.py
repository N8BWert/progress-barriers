...
# The barrier to use in the experiment. Options are "progress", "decentralized", "centralized", "double", and "increasing".
BARRIER: str = "decentralized"
# The number of robots to use in the experiment
NUM_ROBOTS: int = 8
# Is this submitted to the Robotarium? If so, save our output file to the appropriate directory
ROBOTARIUM_SUBMISSION: bool = True
...
def main():
    ...
    while np.linalg.norm(x_si - goal_poses[:2, :]) > GOAL_TOLERANCE:
        ...
        # Apply barriers
        match barrier:
            case "progress" | "decentralized" | "double" | "increasing" | "local-increasing":
                for i in range(num_robots):
