"""
Random Goals Experiment for the Progress Barrier Project

Robots start at random positions and must continually swap positions with each other
using their barrier certificates to avoid collisions.
"""
...
def parse_cli_args() -> tuple[str, bool, bool, bool, int, int]:
    parser = argparse.ArgumentParser(
        description="Run the Random Goals Experiment for the Progress Barrier Project"
    )
    parser.add_argument(
        "-b",
        "--barrier",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="The specific random goal experiment to run.",
    )
    parser.add_argument(
        "--skip-initialization",
        action=argparse.BooleanOptionalAction,
    ...
    return args.barrier, bool(args.hide_figure), bool(args.run_in_real_time), bool(args.skip_initialization), int(args.num_robots), int(args.experiment_num)

def draw_goal_patches(axes: Axes, goal_poses: np.ndarray, goal_patches: list[patches.Rectangle]):
    if len(goal_patches) == 0:
        for i in range(goal_poses.shape[1]):
            goal_anchor = goal_poses[:2, i] - np.array([0.1, 0.1])
        ...
        for i in range(goal_poses.shape[1]):
            goal_patches[i].set_xy((goal_poses[0, i] - 0.1, goal_poses[1, i] - 0.1))

def main():
    barrier, hide_figure, run_in_real_time, skip_initialization, num_robots, experiment_num = parse_cli_args()
    print(f"Using {num_robots} robots with the {barrier} barrier certificate.")

    goals = np.load(f"random_goals/goal_poses_{experiment_num}.npy")
    r = Robotarium(
        number_of_robots=num_robots,
        show_figure = not hide_figure,
        sim_in_real_time = run_in_real_time,
        initial_conditions = goals[0, :, :num_robots],
        skip_initialization = skip_initialization,
    )
    axes = r._axes_handle
    goal_patches = []
    draw_goal_patches(axes, goals[1, :, :num_robots], goal_patches)
    ...
    elapsed_time = 0.0
    last_time = time.time()
    completion_times = np.full((goals.shape[0] - 1, num_robots), np.nan)
    for goal_idx in range(1, goals.shape[0]):
        complete_ids.clear()
        draw_goal_patches(axes, goals[goal_idx, :, :num_robots], goal_patches)
        if barrier == "progress":
            for i in range(num_robots):
        ...
        while np.linalg.norm(x_si - goals[goal_idx, :2, :num_robots]) > GOAL_TOLERANCE:
            ...
            dxi = controller(x_si, goals[goal_idx, :2, :num_robots])
            # Apply barriers
            match barrier:
                ...
                case "progress":
                    for i in range(num_robots):
                        ...
                        barrier_certificates[i].show(x_si[:, i], axes)
            if barrier == "double":
                dxi = centralized_barrier(x_si, dxi)
            dx = si_to_uni(dxi, x)
